...
class modules_structure_tpl {
	...
	function readTemplate($tplType='', $tplName='', $tplPostfix='')
	{
		global $system;
	...
	function readTemplateFile( $isPostfix=true ){
		...
		$isPostfix = $isPostfix && $system['tplLevel'][ modules_structure_tpl::getTplLevelLast() ]['tplPostfix']!='';
		...
		*/
		...
		$tplFileDir = '../site/templates/'.$tplFile.'.tpl';
			...
			else
			{
				...
				if( $isPostfix )
				{
					...
					$system['tplFile'][$tplFile] = 
						&$system['tplFile'][ modules_structure_tpl::readTemplateFile(false) ];	
				}
				else
				{
					//<<error
