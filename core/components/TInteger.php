...
class components_TInteger{
	...
	function edit($name,$parentId,$title){
		...
		$data_child_element=sys::sql("SELECT `id`,`data` FROM `prefix_TInteger` WHERE `name`='$name' AND `parent_id`='$parentId';",0);
		$data=mysql_fetch_array($data_child_element);

		$SEND['title'] = $title;
		$SEND['name'] = $name;
		$SEND['data'] = $data['data'];
		$out = admin::draw('TInteger/editDialog',$SEND);
	...
	function save($POST,$FILES, $param=''){
		$result = sys::sql("SELECT `data` FROM `prefix_TInteger` WHERE `name`='".$POST['dataName']."' AND `parent_id`='".$POST['parent_id']."';",0);
		if (mysql_num_rows($result)==0){
			$POST['parentId']=$POST['parent_id'];
			components_TInteger::createStr($POST);
		}
		$result = sys::sql("UPDATE `prefix_TInteger` SET `data` = '".$POST['data']."' WHERE `name`='".$POST['dataName']."' AND `parent_id`='".$POST['parent_id']."';",0);
		if ($param=='client'){
			return;
		} else {
		}
	...
	function createStr($POST){
		components_TInteger::createTable();
		$result = sys::sql("SELECT `id` FROM `prefix_TInteger` WHERE `name`='".$POST['dataName']."' AND `parent_id`='".$POST['parentId']."'",0);
		if (mysql_num_rows($result)=='0'){
			$result = sys::sql("INSERT INTO `prefix_TInteger` ( `id` , `name` , `parent_id` , `data` ) VALUES ('', '".$POST['dataName']."', '".$POST['parentId']."', '');",0);
		};
	}
	...
	function createTable(){
		$query=sys::sql("
			CREATE TABLE IF NOT EXISTS `prefix_TInteger` (
